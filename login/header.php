<!DOCTYPE html>
<html lang="ar" dir="rtl">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../css/bootstrap.min.css">
    <link rel="stylesheet" href="../css/fontawesomefree.min.css">
    <link rel="stylesheet" href="../css/style.css">
    <title>حافظ</title>
</head>

<body>
    <header>
        <a href="http://127.0.0.1/mysite/" class="logo"><i class="fa-solid fa-house-chimney"></i> حافظ </a>
        <nav class="navigation">
            <a href="http://127.0.0.1/mysite/personal-page.php"><i class="fa-solid fa-user"></i></a>
            <a href="#" onclick="openNav()"><i class="fa-solid fa-bars"></i></a>
        </nav>
    </header>
    <div id="mySidenav" class="sidenav">
        <a href="javascript:void(0)" class="closebtn" onclick="closeNav()">&times;</a>
        <a href="http://127.0.0.1/mysite/">الرئيسية</a>
        <a href="http://127.0.0.1/mysite/personal-page.php">الصفحة الشخصية</a>
        <a href="http://127.0.0.1/mysite/#aboutUs">من نحن</a>
        <a href="http://127.0.0.1/mysite/#contact">تواصل معنا</a>

